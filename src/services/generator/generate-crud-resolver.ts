...
export function generateCrudResolver(model: ISingleErModel) {
  ...
  return `import { Arg, Args, Mutation, Query, Info, ID, Ctx, Resolver, Root, FieldResolver, Int } from 'type-graphql';
...
export class ${modelName}CrudResolver {
  ...
  async delete${plural(
    modelName,
  )}(@Arg('ids', () => [ID]) ids: Array<EntityId>, @Ctx() ctx: IRequestContext): Promise<boolean> {
