...
export function generateField(field: IFieldDefinition) {
  return `  ${generateFieldDeco(field)}
...
export function generateInput(model: ISingleErModel, type: 'edit' | 'create' | 'nested' | 'search' | 'searchOrder') {
  ...
  const manyToOneFields = manyToOneRelations
    .map(
      (r): IFieldDefinition => ({
        dbType: undefined,
        name: r.myName,
  ...
  const oneToOneFields = oneToOneRelations
    .map(
      (r): IFieldDefinition => ({
        dbType: undefined,
        name: r.myName,
  ...
  return `import { Field, ID, InputType } from 'type-graphql';
