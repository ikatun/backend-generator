import { kebabCase, lowerFirst, uniq, upperFirst } from 'lodash';
...
export function generateToOneInitialization(relation: ISingleErRelation, modelName: string) {
  ...
  return `export async function update${upperFirst(
    relation.myName,
  )}Relation(${modelVariableName}: ${modelName}, ${fieldName}: ${
    relation.otherTypeName
  }NestedInput | null | undefined, context: IRequestContext) {
  const ${existingRelationName} = await ${modelVariableName}.${relation.myName};

  if (${fieldName} === null) {
    ${
      relation.optional
        ? `${modelVariableName}.${relation.myName} = Promise.resolve(null);`
        : `throw new Error('${relation.myTypeName}.${relation.myName} cannot be null')`
    }
    ...
    ${modelVariableName}.${relation.myName} = asPromise(await new ${
    relation.otherTypeName
...
export function generateUpdateOperations(model: ISingleErModel) {
  ...
  return `import { asPromise } from '../../../shared/as-promise';
